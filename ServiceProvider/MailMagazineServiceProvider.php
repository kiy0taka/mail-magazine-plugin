...
class MailMagazineServiceProvider implements ServiceProviderInterface
{
    ...
    {
        // メルマガテンプレート用リポジトリ
        $app['eccube.plugin.mail_magazine.repository.mail_magazine'] = function () use ($app) {
            return $app['orm.em']->getRepository('Plugin\MailMagazine\Entity\MailMagazineTemplate');
        };

        // 配信履歴用リポジトリ
        $app['eccube.plugin.mail_magazine.repository.mail_magazine_history'] = function () use ($app) {
            return $app['orm.em']->getRepository('Plugin\MailMagazine\Entity\MailMagazineSendHistory');
        };

        // Customer用リポジトリ
        $app['eccube.plugin.mail_magazine.repository.mail_magazine_customer'] = function () use ($app) {
            return new MailMagazineCustomerRepository($app['orm.em'], $app['orm.em']->getMetadataFactory()->getMetadataFor('Eccube\Entity\Customer'));
        };

        // 新規会員登録/Myページ
        $app['eccube.plugin.mail_magazine.repository.mail_magazine_mailmaga_customer'] = function () use ($app) {
            return $app['orm.em']->getRepository('Plugin\MailMagazine\Entity\MailmagaCustomer');
        };

        // イベント
        $app['eccube.plugin.mail_magazine.event.mail_magazine'] = function () use ($app) {
            return new MailMagazine($app);
        };
        $app['eccube.plugin.mail_magazine.event.mail_magazine_legacy'] = function () use ($app) {
        ...
        // サービス
        // -----------------------------
        $app['eccube.plugin.mail_magazine.service.mail'] = function () use ($app) {
